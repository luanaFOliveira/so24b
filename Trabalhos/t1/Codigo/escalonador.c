...
void escalonador_remove_processo(escalonador_t *self, processo_t *processo) {
    ...
    do {
        if (p->processo == processo) {
            // Caso o nó a ser removido seja o único na lista
            if (p == self->inicio && p == self->fim) {
                self->inicio = self->fim = NULL;
            }
            // Caso o nó a ser removido seja o primeiro (inicio)
            else if (p == self->inicio) {
                self->inicio = p->prox;
                self->fim->prox = self->inicio; // Atualiza o fim para apontar para o novo inicio
            }
            // Caso o nó a ser removido seja o último (fim)
            else if (p == self->fim) {
                self->fim = ant;
                self->fim->prox = self->inicio; // Atualiza o fim para apontar para o inicio
            }
            // Caso o nó a ser removido seja algum nó intermediário
            else {
                ant->prox = p->prox;
            }
            free(p);
